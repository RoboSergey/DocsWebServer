...
class TestVersions:
    ...
    async def test_list_versions_ordered_descending(self, client):
        doc_id = await self._create_doc_with_content(client)
        ...
        res = await client.get(f"/api/documents/{doc_id}/versions")
        nums = [v["version_num"] for v in res.json()["versions"]]
    ...
    async def test_restore_nonexistent_version(self, client):
        doc_id = await self._create_doc_with_content(client)
        res = await client.post(f"/api/documents/{doc_id}/versions/999/restore")
        assert res.status_code == 404
